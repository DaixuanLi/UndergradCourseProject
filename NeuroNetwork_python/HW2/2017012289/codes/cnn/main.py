...
def lc_plot(train_loss_list,train_acc_list,test_loss_list,test_acc_list, plot_str):
    ...
    pdf = PdfPages('./imgs/' + plot_str + ',plot.pdf')
    ...
    loss_frame = {'Epoch':iter_list,
                  'Loss':train_loss_list+test_loss_list,
                  'Dataset':type_list
                  }
    loss_frame = DataFrame(loss_frame)

    g = sns.lineplot(x="Epoch", y="Loss", hue='Dataset', style='Dataset',
    ...
    g = sns.lineplot(x="Epoch", y="Acc", hue='Dataset', style='Dataset',
                     data=loss_frame, legend='full', err_style='bars', palette=palette, linewidth=2,
                     err_kws={'elinewidth': 2}, ax=ax1)
...
if __name__ == '__main__':
	...
	plot_str = "BatchNorm="+str(False) +",drop=" + str(args.drop_rate) +",batch_size=" + str(args.batch_size)
	...
	if args.is_train:
		...
		cnn_model = Model(drop_rate=args.drop_rate, use_bn=False)
		cnn_model.to(device)
		print(cnn_model)
		print(plot_str)
		...
		lc_plot(train_loss_list, train_acc_list, test_loss_list, test_acc_list, plot_str)
