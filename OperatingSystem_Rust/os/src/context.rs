...
impl Context {
    ...
    pub unsafe fn new_forked_thread(_tf:&mut TrapFrame, satp:usize, kstack_top: usize) -> Context {
        ContextContent::new_forked_thread(_tf,satp).push_at(kstack_top)
    }
...
impl ContextContent {
    ...
    fn new_forked_thread(_tf:&mut TrapFrame, satp:usize) -> ContextContent {
        let mut content = ContextContent {
            ra: __trapret as usize,
            satp,
            s : [0;12],
            tf: {
                ...
                tf.x[10] = 0;
                tf.sepc = _tf.sepc;
                tf.sstatus = _tf.sstatus;
                tf.scause = _tf.scause;
                tf.stval = _tf.stval;
                tf
            },
        };
