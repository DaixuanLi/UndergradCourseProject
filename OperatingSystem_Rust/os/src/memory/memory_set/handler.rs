...
pub trait MemoryHandler: Debug + 'static {
    ...
    fn clone_map(&self, pt: Arc<Mutex<PageTableImpl>>, src_pt: Arc<Mutex<PageTableImpl>>, addr: usize, attr: &MemoryAttr,);
}
...
impl MemoryHandler for ByFrame {
    ...
    fn clone_map(&self, pt: Arc<Mutex<PageTableImpl>>, src_pt: Arc<Mutex<PageTableImpl>>, addr: usize, attr: &MemoryAttr,){
        self.map(pt.clone(), addr, attr);
        let data = src_pt.lock().get_page_slice_mut(addr);
        pt.lock().get_page_slice_mut(addr).copy_from_slice(data);
    }
}
